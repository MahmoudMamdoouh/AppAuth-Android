...
/**
 * An OpenID Connect ID Token. Contains claims about the authentication of an
 * End-User by an
 * Authorization Server. Supports parsing ID Tokens from JWT Compact
 * Serializations and validation
...
class IdToken {
    ...
    static IdToken from(String token) throws JSONException, IdTokenException {
        ...
        // We ignore header contents, but parse it to check that it is structurally
        // valid JSON
    ...
    void validate(@NonNull TokenRequest tokenRequest,
        ...
        // Validates that the issuer in the ID Token matches that of the discovery
        // document.
        ...
        // Validates that the aud (audience) Claim contains the client ID, or that the
        // azp
        ...
        // communication of the ID Token from the Token Endpoint to the Client, and we
        // are
        // exercising the option to use TLS server validation instead of checking the
        // token
        ...
        // Validates that the issued at time is not more than +/- 10 minutes on the
        // current
        // time.
        // if (Math.abs(nowInSeconds - this.issuedAt) > TEN_MINUTES_IN_SECONDS) {
        // throw
        // AuthorizationException.fromTemplate(GeneralErrors.ID_TOKEN_VALIDATION_ERROR,
